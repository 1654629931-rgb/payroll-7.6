;(function () {
  ...
  function defaultData() {
    return {
      version: APP_VERSION,
      categories: [
        { id: uid(), name: '理账项目', mode: 'fixed', value: 300 },
        { id: uid(), name: '审计项目', mode: 'rate', value: 8 },
        { id: uid(), name: '报税项目', mode: 'fixed', value: 150 },
        { id: uid(), name: '工商注册', mode: 'fixed', value: 200 },
      ],
      employees: [],
      projects: [],
      monthAdjustments: {},
  ...
  function renderEmployees() {
    if (!state.employees.length) {
      els.employeeBody.innerHTML = `
        <tr><td colspan="4"><div class="hint">暂无员工档案，请点击右上角“新增员工”。</div></td></tr>
      `
      return
    }
  ...
  function generateDemoData() {
    ...
    next.employees = [
      { id: uid(), name: '张会计', role: '外勤会计', baseSalary: 6500, phone: '13800000001', note: '' },
      { id: uid(), name: '李会计', role: '审计助理', baseSalary: 7200, phone: '13800000002', note: '' },
      { id: uid(), name: '王会计', role: '财顾', baseSalary: 9000, phone: '13800000003', note: '' },
    ]

    const month = currentMonth()
    const categoryAudit = next.categories.find((c) => c.name.includes('审计')) || next.categories[1]
    const categoryBook = next.categories.find((c) => c.name.includes('理账')) || next.categories[0]
    ...
    next.projects = [
      {
        ...
        compliancePassed: false,
        settled: false,
        ruleMode: categoryAudit.mode,
        ruleValue: categoryAudit.value,
        participants: [
          { id: uid(), employeeId: next.employees[1].id, mode: 'ratio', value: 60 },
          { id: uid(), employeeId: next.employees[2].id, mode: 'ratio', value: 40 },
        ],
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      },
      {
        ...
        settled: true,
        ruleMode: categoryBook.mode,
        ruleValue: categoryBook.value,
        participants: [{ id: uid(), employeeId: next.employees[0].id, mode: 'fixed', value: 300 }],
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      },
    ]

    next.monthAdjustments = {
      [month]: {
        [next.employees[0].id]: { baseSalary: 6500, attendance: -200, socialInsurance: 580 },
        [next.employees[1].id]: { baseSalary: 7200, attendance: 0, socialInsurance: 640 },
        [next.employees[2].id]: { baseSalary: 9000, attendance: 300, socialInsurance: 820 },
      },
    }
  ...
  function bindEvents() {
    ...
    els.employeeBody.addEventListener('click', (event) => {
      ...
      if (action === 'employee-delete') {
        const usedProjects = state.projects.filter((p) => p.participants.some((r) => r.employeeId === id))
        const tip = usedProjects.length
          ? `该员工已参与 ${usedProjects.length} 个项目，删除后这些项目将移除其分配。`
          : ''
        if (!window.confirm(`确认删除员工“${employee.name}”吗？${tip}`)) return

        const nextState = deepClone(state)
        nextState.employees = nextState.employees.filter((e) => e.id !== id)
        nextState.projects = nextState.projects.map((p) => ({
          ...p,
          participants: p.participants.filter((r) => r.employeeId !== id),
        }))
        if (nextState.monthAdjustments) {
          Object.keys(nextState.monthAdjustments).forEach((month) => {
            if (nextState.monthAdjustments[month]) {
              delete nextState.monthAdjustments[month][id]
            }
          })
        }
    ...
    els.demoData.addEventListener('click', generateDemoData)
